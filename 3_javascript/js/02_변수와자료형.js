...
// 변수의 선언 
// 메모리에 값을 계속해서 바꿔가면서 저장할 공간을 만들고 이름을 붙이는 것
// <작성법>
// 변수종류 변수명;
 var number1; //unrefind 

// 초기화(== 변수에 처음 값을 할당하는 과정)
// 선언된 변수에 값을 처음 대입할 때 쓰는 것
// <작성법>
// 변수명=값;
 number1 = 10; 
...
// 문자열 + 문자열/숫자/변수 == 이어쓰기
console.log("number1의 값 : " + number1);// ""는 생략
console.log("number2의 값 : " + number2);// ""는 생략
console.log("100" + 100); //"100100"

// 2) 괄호 내부에, 를 작성해 각각의 값을 독립적으로 출력하기
// 형변환 없이 출력되어 문자열 따로, 다른 데이터타입 따로 출력됨
console.log("number1 : ", number1 ,  "/ numvber2 : " , number2) 
// 같은 쌍따옴표에 들어온게 아님. , 은 생략되어 출력
...
// PI = 1.23; 상수에 새로운 값을 대입할 수 없는데, 대입하려고 해서 에러.
...
// : let 과 const로 선언된 변수 상수는 변수가 선언된 블록(중괄호{}) 내부에서만
// 유효하다 라는 것을 의미
// 즉, 코드 블록 내부에서 선언한 변수는 해당 블록 외부에서는 접근할 수 없다.
// 함수 레벨 scope (var)
...
const foo2 = 456;//전역 변수
 ...
 { // 블록 레벨 scope 
  // let num1 = 789;
  // const num2 = 0; 
  //02_변수와자료형.js:110 Uncaught 
  // ReferenceError: num1 is not defined
  // 둘다 블록 레벨 스코프를 가지고 있기 때문에 접근 못함
}
...
//  블록 레벨 scope를 무시하고, 변수가 선언된 함수 전체에서 유효함.
...
// 가능 함수레벨 스코프인 var 는 블록을 무시하기 때문에

// 그럼 함수 안에서는 어떻게 될까?

//함수의 선언
function example() {// 이렇게 함수 형태로 만들어진 코드블록은 함수레벨scope}
...
// -> var 는 블록레벨만 무시할 뿐, 함수레벨은 무시되지 않음
...
// 변수의 선언과 초기화
var a = 10;
// 쵝화는 원래 코드의 위치에서 이루어짐

console.log(a); // 10


// 2. let/const 의 호이스팅
// : let.const 로 선언된 변수는 선언만 해당 스코프의 
// 최상단으로 호이스팀 됨.
// var 와 달리, let/const로 선언된 변수는 초기화가
// 초기화가 이루어지기 전에는 사용될 수 없음


/**
 * TDZ
 * 시간적으로 죽은 구역..=일시적인 사각지대
 * - 코드 실행 과정에서 특정 시점까지
 * 변수가 접근 불가능한 상태를 의미함.
...
console.log("nubers :", numbers, typeof numbers);
...
// 2) JS 객체
// 값을 K:V(키벨류) (Map) 형식으로 여러개 저장하는 형태
// -> { K:V, K:V, K:V, K:V..}
...
// 함수를 정의할 때 사용(전달)하는 변수 : 매개변수 (Pareameter)
...
*/
console.log( typeof sumFn ); //fuction
