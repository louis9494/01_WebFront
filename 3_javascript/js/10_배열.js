...
function selectMenu() {
  ...
  // 점심 메뉴로 초기화된 배열 생성
  const menu = ["돈까스", "짬뽕", "라면", "김밥",
                "순대국", "삼겹살", "초밥", "우동",
                "규카츠", "과일", "푸딩"];

  // menu 배열 index 범위 내에서 난수 생성
  const randomNumber = (Math.random() * menu.length);

  // 난수 번째 undex 요소값을 화면에 출력하기만 하면 끝
...
function check6() {
  ...
  for(let row = 0; row < 5; row++) { // 행 제어
    ...
    for(let col = 0; col < 5; col++) { // 열 제어
      arr[row][col] = count; // row행 col열에 count 를 eodlq
      count += 2; // count를 2증가
